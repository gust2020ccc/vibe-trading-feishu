...
def _get_session_service():
    ...
    # Wire up usage tracking callback
    _usage_svc = _get_usage_service()
    if _usage_svc:
        _session_service.on_attempt_complete = lambda **kw: _usage_svc.record_attempt_usage(**kw)
...
def _get_usage_service():
    """Lazy-init UsageQuotaService singleton."""
    global _usage_service
    if _usage_service is not None:
        return _usage_service
    try:
        from src.usage.service import UsageQuotaService
        _usage_service = UsageQuotaService()
    except Exception:
        import logging
        logging.getLogger(__name__).exception("Failed to init UsageQuotaService")
        _usage_service = None
    return _usage_service
...
def _get_channel_runtime():
    ...
    _channel_runtime = ChannelRuntime(
        ...
        quota_checker=_get_usage_service().check_and_acquire if _get_usage_service() else None,
        quota_releaser=_get_usage_service().release_concurrency if _get_usage_service() else None,
    )
