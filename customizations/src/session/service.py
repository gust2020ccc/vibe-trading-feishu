...
class SessionService:
    ...
    def __init__(
        ...
        # Optional callback invoked after each attempt completes (for usage tracking).
        # Signature: (sender_id, channel, session_id, attempt_id, run_dir, status) -> None
        self.on_attempt_complete: Any = None
    ...
    async def send_message(
        ...
        *,
        ...
        Args:
            ...
            sender_id: The channel sender ID (e.g. Feishu open_id) for usage tracking.
            channel: The channel name (e.g. "feishu") for usage tracking.
        ...
        if sender_id:
            session.config["sender_id"] = sender_id
        if channel:
    ...
    async def _run_attempt(self, session: Session, attempt: Attempt, *, include_shell_tools: bool = False) -> None:
        ...
        try:
            ...
            # Usage tracking callback (invoked after attempt is fully resolved)
            if self.on_attempt_complete:
                try:
                    self.on_attempt_complete(
                        sender_id=session.config.get("sender_id", ""),
                        channel=session.config.get("channel", ""),
                    ...
                    logger.debug("on_attempt_complete callback failed", exc_info=True)
            ...
            # Usage tracking callback on failure (release concurrency slot)
            if self.on_attempt_complete:
                try:
                    self.on_attempt_complete(
                        sender_id=session.config.get("sender_id", ""),
